...
from . import PodcastFile, upload_to_server, generate_html, util
...
COURSES_NAMES = util.catalog_names()['corsi'].keys()
TEACHERS_NAMES = util.catalog_names()['docenti'].keys()
...
class MainFrame(tk.Frame):
    ...
    def get_text_lines(self):
        """Return the names of the track from the text_box."""
    ...
    def _main(self):
        ...
        for valid_file in enumerate(valid_files, 1):
            INFO_LOGGER.info('Creazione podcast %s',
...
class MainPage(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.page_main = ttk.Frame(window_main, width=1000, height=600)
        self.page_main.grid(column=0, row=0)
        self.page_main.grid_propagate(False)
        window_main.add(self.page_main, text='Main')
        self.labels()

        self.page_audio = ttk.Frame(window_main, width=1000, height=600)
        self.page_audio.grid(column=0, row=0)
        self.page_audio.grid_propagate(False)
        window_main.add(self.page_audio, text='Audio')

        self.ops_image(self.page_audio)

        audio = AudioFrame(self.page_audio)
        audio.place(x=0, y=0)
        audio_label = ttk.Label(self.page_audio,
                                text='OPS :(\nWORK IN PROGRESS.',
                                style='options.TLabel')
        audio_label.place(x=50, y=100)

        self.page_catalog = ttk.Frame(window_main, width=1000, height=600)
        self.page_catalog.grid(column=0, row=0)
        self.page_catalog.grid_propagate(False)
        window_main.add(self.page_catalog, text='Catalogo Nomi')

        self.ops_image(self.page_catalog)

        catalogo_label = ttk.Label(self.page_catalog,
                                   text='OPS :(\nWORK IN PROGRESS.',
                                   style='options.TLabel')
        catalogo_label.place(x=50, y=100)

        window_main.pack()
        self.window_error = MainFrame(
            self.page_main, audio, width=670, height=360)
        self.window_error.place(x=5, y=210)

        align_btn = ttk.Button(self.page_main, text='Allinea finestre',
    ...
    def labels(self):
        ...
        ttk.Label(self.page_main, text='Podcast files:',
